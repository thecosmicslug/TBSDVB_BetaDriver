...
u8 DMD_I2C_SLAVE[DMD_MAX_DEVICE][DMD_REG_BANK] ={ {0x18,0x10} , {0x19,0x11} , {0x1A,0x12} , {0x1B,0x13} };


u32 MN88436_REG_AUTOCTRL_SIZE = 673;

u8  MN88436_REG_AUTOCTRL[] = {
...
u8 MxLWare603DrvVersion[] = {2, 1, 1, 3, 0}; 


struct MXL603_REG_CTRL_INFO_T MxL603_OverwriteDefaults[] = 
{
  {0x14, 0xFF, 0x13},
  {0x6D, 0xFF, 0x8A},
  {0x6D, 0xFF, 0x0A},
...
struct MXL603_REG_CTRL_INFO_T MxL603_DigitalIsdbtAtsc[] = 
{
  {0x0C, 0xFF, 0x00},
  {0x13, 0xFF, 0x04},
  {0x53, 0xFF, 0xFE},
...
struct MXL603_REG_CTRL_INFO_T MxL603_DigitalDvbc[] = 
{
  {0x0C, 0xFF, 0x00},
  {0x13, 0xFF, 0x04},
  {0x53, 0xFF, 0x7E},
...
// Digital DVB-T 6MHz application mode setting
 struct MXL603_REG_CTRL_INFO_T MxL603_DigitalDvbt[] = 
{
  {0x0C, 0xFF, 0x00},
  {0x13, 0xFF, 0x04},
  {0x53, 0xFF, 0xFE},
...
struct mndmd_state
{
	...
	struct DMD_PARAMETER_t param;

};

/* Calculate Log10 */
static u32 log10_easy( u32 cnr )
{
	u32	c;
	s32	ret;
	c = 0;
...
/*! Write 1byte */
static int DMD_I2C_Write(struct dvb_frontend* fe,u8	slvadr , u8 adr , u8 data )
{
...
/*! Read 1byte */
static int DMD_I2C_Read(struct dvb_frontend* fe,u8	slvadr , u8 adr , u8 *data )
{
	//TODO:	Please call I2C 1byte Write API
	struct mndmd_state *state = fe->demodulator_priv;
...
static int DMD_I2C_MaskWrite(struct dvb_frontend* fe,u8	slvadr , u8 adr , u8 mask , u8  data )
{
	u8	rd;
	int ret;
...
static int DMD_send_registers(struct dvb_frontend* fe, u8* slaveset , u8* regset )
{
	u32	i;
	for(i=0;;)
	{
...
static int MXL603_Write( struct mndmd_state *state , u8 RegAddr, u8 RegData)
{
	int ret;
...
static int MXL603_Read(struct mndmd_state *state , u8 RegAddr , u8 *rdata )
{
	int ret;
...
static int MxLWare603_OEM_WriteRegister(struct dvb_frontend *fe, u8 RegAddr, u8 RegData)
{
	int Status = 0;
...
static int MxLWare603_OEM_ReadRegister(struct dvb_frontend *fe, u8 RegAddr, u8 *DataPtr)
{
	int Status = 0;
	struct mndmd_state *state= fe->demodulator_priv;
...
static int MxL603_Ctrl_ProgramRegisters(struct dvb_frontend *fe,  struct MXL603_REG_CTRL_INFO_T* ctrlRegInfoPtr)
{
	int status = MXL_TRUE;
	u16 i = 0;
	u8 tmp = 0;
...
static int MxLWare603_API_CfgDevSoftReset(struct dvb_frontend *fe)
{
...
static int MxLWare603_API_CfgDevOverwriteDefaults(struct dvb_frontend *fe, enum MXL_BOOL singleSupply_3_3V)
{
	int status = MXL_SUCCESS;
	u8 readData = 0;
...
static int MxLWare603_API_CfgDevXtal(struct dvb_frontend *fe,  struct MXL603_XTAL_SET_CFG_T xtalCfg)
{
	int status = MXL_SUCCESS;
	u8 control = 0;
...
static int MxLWare603_API_CfgDevPowerMode(struct dvb_frontend *fe, enum MXL603_PWR_MODE_E powerMode)
{
	int status = MXL_SUCCESS;
...
static int MxLWare603_API_CfgDevGPO(struct dvb_frontend *fe, enum MXL603_GPO_STATE_E gpoState)
{
	int status = MXL_SUCCESS;
	u8 regData = 0;
...
static int MxLWare603_API_ReqDevVersionInfo(struct dvb_frontend *fe,struct MXL603_VER_INFO_T* mxlDevVerInfoPtr)
...
static int MxLWare603_API_CfgTunerMode(struct dvb_frontend *fe,struct MXL603_TUNER_MODE_CFG_T tunerModeCfg)
...
static int MxLWare603_API_CfgTunerAGC(struct dvb_frontend *fe, struct MXL603_AGC_CFG_T agcCfg)
{
	int status = MXL_SUCCESS;
	u8 regData = 0; 
...
static int MxLWare603_API_CfgTunerLoopThrough(struct dvb_frontend *fe, enum MXL_BOOL loopThroughCtrl)
{
	int status = MXL_SUCCESS;
	u8 regData;
...
static int MxLWare603_API_CfgTunerChanTune(struct dvb_frontend *fe, struct MXL603_CHAN_TUNE_CFG_T chanTuneCfg)
	...
	if (chanTuneCfg.startTune == MXL_ENABLE)
	{
		if (chanTuneCfg.signalMode <= MXL603_DIG_J83B) 
		{
			...
			case MXL603_TERR_BW_8MHz:
				status |= MxLWare603_OEM_WriteRegister(fe, CHAN_TUNE_BW_REG, (u8)chanTuneCfg.bandWidth);
				...
				// Set RF  
				status |= MxLWare603_OEM_WriteRegister(fe, CHAN_TUNE_LOW_REG, (u8)(freq & 0xFF));
				status |= MxLWare603_OEM_WriteRegister(fe, CHAN_TUNE_HI_REG, (u8)((freq >> 8 ) & 0xFF));
				break;
...
static int MxLWare603_API_CfgTunerIFOutParam(struct dvb_frontend *fe, struct MXL603_IF_OUT_CFG_T ifOutCfg)
{
	u16 ifFcw;
	int status = MXL_SUCCESS;
	u8 readData = 0;
...
static int MxLWare603_API_ReqTunerAGCLock(struct dvb_frontend *fe, enum MXL_BOOL* agcLockStatusPtr)
{
	 int status = MXL_SUCCESS;
	u8 regData = 0;
	 enum MXL_BOOL lockStatus = MXL_UNLOCKED;
	...
	if (agcLockStatusPtr)
	{
		...
		*agcLockStatusPtr =  lockStatus;
		...
		//printk(" Agc lock = %d", (u8)*agcLockStatusPtr); 
	}
	else
		status = MXL_INVALID_PARAMETER;
...
static int MxLWare603_API_ReqTunerLockStatus(struct dvb_frontend *fe,  enum MXL_BOOL* rfLockPtr, 
														 	 enum MXL_BOOL* refLockPtr)
{
	int status = MXL_SUCCESS;
	u8 regData = 0;
	enum MXL_BOOL rfLockStatus = MXL_UNLOCKED;
	enum MXL_BOOL refLockStatus = MXL_UNLOCKED;
...
static int MxLWare603_API_ReqTunerRxPower(struct dvb_frontend *fe, s16* rxPwrPtr)
{
	int status = 0;
	u8 regData = 0;
	u16 tmpData = 0;
...
static int MXL603_init(struct dvb_frontend *fe)
{
	int status; 
//	u8 devId;
	enum MXL_BOOL singleSupply_3_3V;
	struct MXL603_XTAL_SET_CFG_T xtalCfg;
	struct MXL603_IF_OUT_CFG_T ifOutCfg;
	struct MXL603_AGC_CFG_T agcCfg;
	struct MXL603_TUNER_MODE_CFG_T tunerModeCfg;
	struct MXL603_VER_INFO_T	mxl603Version;
...
static int Mxl603SetFreqBw(struct dvb_frontend *fe,u32 freq,enum MXL603_SIGNAL_MODE_E signalMode)
{
	int status; 

	enum MXL_BOOL refLockPtr;
	enum MXL_BOOL rfLockPtr;
	struct MXL603_CHAN_TUNE_CFG_T chanTuneCfg;
	u32 rf;
//	devId = MXL603_I2C_ADDR;
...
static int  DMD_open(  struct DMD_PARAMETER_t* param)
{
	int i;
...
/* **************************************************** */
static int DMD_device_init(struct dvb_frontend* fe,struct DMD_PARAMETER_t* param )
{
	int	ret;
	ret = 0;
...
static int DMD_device_load_pseq( struct dvb_frontend* fe, struct DMD_PARAMETER_t* param )
{
	int ret = 0;
	u8	data;
	u32	i;
...
static int	DMD_device_set_TCB_mode(struct dvb_frontend* fe,struct DMD_PARAMETER_t* param)
{
	int	ret;
	u8	bank;
	u8	tcbcom,tcbadr,tcbset;
...
static int DMD_set_ts_output(struct dvb_frontend* fe,struct DMD_PARAMETER_t* param,  enum DMD_TSCLK_POLARITY ts_clk_polarity )
{  
	switch(param->ts_out)
	{
...
/*! Soft Reset */
/* **************************************************** */
static int DMD_device_reset(struct dvb_frontend* fe,struct DMD_PARAMETER_t* param )
{
	int	ret;
	switch( param->system ){
		case DMD_E_ATSC:
...
static int DMD_device_post_tune(struct dvb_frontend* fe, struct DMD_PARAMETER_t* param )
{
...
static int DMD_scan_vq(struct dvb_frontend* fe, struct DMD_PARAMETER_t *param )
{
...
static int DMD_device_scan(struct dvb_frontend* fe, struct DMD_PARAMETER_t* param )
{
	int	ret = 1;
	switch( param->system )
	{
...
static int	DMD_device_set_system(struct dvb_frontend* fe, struct DMD_PARAMETER_t* param)
{
	int	ret;
	ret = 0;
...
static int DMD_device_set_echo_enhance(struct dvb_frontend* fe,struct DMD_PARAMETER_t* param, enum DMD_ECHO_PERFORMANCE_SET echo_out )
{    
	switch(echo_out)
       {
...
static int DMD_set_system( struct dvb_frontend* fe,struct DMD_PARAMETER_t* param )
{

		if( DMD_device_set_system(fe, param ) == 1 )
		{
		...
		if( DMD_set_ts_output(fe,param, param->ts_out ) == 1 ){
			printk("ERROR:set ts output");
			return 1;
		}
		
		/* 20120801, troy, set echo performance */
		if (DMD_device_set_echo_enhance(fe, param, param->echo_set) == 1 )
		{
...
static int DMD_init( struct dvb_frontend* fe,struct DMD_PARAMETER_t* param )
{
	...
	if (DMD_device_set_TCB_mode( fe,param ) == 1 )
	{
...
{	
	...
	struct DMD_PARAMETER_t param =state->param;
	DMD_open(&param);
	DMD_init( fe,&param);
	MXL603_init(fe);
...
static int DMD_set_parameters(struct dvb_frontend* fe)
{
	...
	struct dtv_frontend_properties *c = &fe->dtv_property_cache;
	struct DMD_PARAMETER_t param =state->param;
	enum MXL603_SIGNAL_MODE_E signalMode = MXL603_DIG_ISDBT_ATSC;
	switch (c->modulation) {
	case VSB_8:
		param.system = DMD_E_ATSC;
		signalMode = MXL603_DIG_ISDBT_ATSC;
		break;
	case QAM_64:
		param.system = DMD_E_QAMB_64QAM;
		signalMode = MXL603_DIG_J83B;		
		break;
	case QAM_256:
	...
	ret=DMD_set_system(fe,&param);
	msleep(20);
	ret=Mxl603SetFreqBw(fe, c->frequency,signalMode);
	msleep(20);
	ret=DMD_device_post_tune(fe, &param );
	msleep(100);
	/* Call Lock/SYNC Status Judgement */
	ret=DMD_device_scan(fe, &param );
	msleep(100);
	return 0;
}
